...
main = src/main.c
objects = src/chunk.c src/debug.c src/line.c src/memory.c src/value.c src/vm.c src/stack.c src/compiler.c src/scanner.c src/object.c src/table.c src/mmm.c src/utils.c src/assembly.c
flags = -std=c2x
...
test-mmm:
	$(MAKE) clean
	gcc $(flags) $(mmm_linker_options) $(trace_flags) -o test-mmm src/mmm_tests.c $(objects) -lm
	./test-mmm
